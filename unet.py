...
class UNet(object):
    ...
    def create_model(self):
        ...
        # lowest layer
        conv1 = keras.layers.Conv2D(1024, 3, activation='relu', padding='same')(inp)
        conv2 = keras.layers.Conv2D(1024, 3, activation='relu', padding='same')(conv1)

        # Upsample and convolutions
        inp = conv2
        for x in range(4):
            filters = 2**(9-x)
            inp = self.upconv_layer(filters, inp, self.skip[-x-1])
    ...
    def conv_layer(self, filters, inp):
        conv1 = keras.layers.Conv2D(filters, 3, activation='relu', padding='same')(inp)
        conv2 = keras.layers.Conv2D(filters, 3, activation='relu', padding='same')(conv1)
    ...
    def upconv_layer(self, filters, inp, skip):
        ...
        cut_skip = keras.layers.Lambda(lambda x: x[:,x_start:x_end, y_start: y_end,:])(skip)

        merge = keras.layers.concatenate([cut_skip, up_conv], axis=-1)
        conv1 = keras.layers.Conv2D(filters, 3, activation='relu', padding='same')(merge)
        conv2 = keras.layers.Conv2D(filters, 3, activation='relu', padding='same')(conv1)
